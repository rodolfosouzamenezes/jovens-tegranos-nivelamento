...
function characteristic(item){
    return `
        ...
        <div class="preview__pokemon">
            <img src="/src/images/stat/stat-${item.avatar}.png" alt="${item.name}" class="preview__image">
        </div>
    ...
    if (pokemon.evolutions.length === 0) {
        ...
        const evolutions = pokemon.evolutions.map((evolution) => `
            <div class="evolutions__item">
                <img src="/src/images/evolution/${((evolution.avatar).replace('+', '-')).replace('+', '-')}" alt="${evolution.name}" class="evolutions__image">
